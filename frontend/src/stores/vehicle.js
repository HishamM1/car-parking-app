...
export const useVehicle = defineStore("vehicle", () => {
    ...
    function storeVehicle() {
        ...
        window.axios.post("vehicles", form).then(()=> { router.push({name: "vehicles.index"})})
        .catch((error)=> { 
    ...
    function updateVehicle(vehicle) {
        ...
        window.axios.put(`vehicles/${vehicle.id}`, form).then(() => router.push({name: "vehicles.index"}))
        .catch((error) => { 
            if (error.response?.status === 422) {
                errors.value = error.response.data.errors;
            }
    ...
    return { form, errors, loading, resetForm, storeVehicle, getVehicles, vehicles, getVehicle, updateVehicle, deleteVehicle }
})